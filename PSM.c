...
void Price(Grpdata* grpdata_ptr,Vdata* vdata_ptr); 
...
/* ------------------------------------------------------- */
	...
	do{
		...
		{ 
			...
			{
				idx[j] = j;
				cf[j] = (1 + mu[J_tld[i][j]])*grpdata_ptr->res[i]*grpdata_ptr->cost[i][J_tld[i][j]] - lmd*grpdata_ptr->cost[i][J_tld[i][j]];
			}
			k_select(cf,idx,N,JT);

			for (int n = 0;n < N; ++n)
			{  
				vdata_ptr->sol[i][n] = J_tld[i][idx[n]];
			}
		}
